      ...
      <div className="relative z-[1] mx-auto max-w-7xl px-5 py-12 sm:px-6 sm:py-16">
        <div className="grid gap-10 border-b border-white/15 pb-12 lg:grid-cols-[1fr_0.78fr] lg:items-end">
          ...
          <div className="lg:justify-self-end">
            <p className="max-w-md text-sm leading-6 text-biome-ink/65">A map-first guide to India&apos;s wildlife, seasons and protected places—built from local data and traceable field sources.</p>
        ...
        <div className="grid gap-10 py-10 sm:grid-cols-2 lg:grid-cols-[1fr_0.55fr_0.55fr]">
          <div>
            ...
            <p className="field-label mt-4 text-biome-ink/45">22.9734° N · 78.6569° E</p>
            <p className="mt-3 max-w-sm text-sm leading-6 text-biome-ink/60">Static by design. No visitor account, live booking database or hidden recommendation engine.</p>
          </div>
          <nav aria-label="Explore">
            <p className="field-label text-biome-accent">Explore</p>
            <div className="mt-4 grid gap-3">
              {explore.map((item) => <Link key={item.href} href={item.href} className="text-sm text-biome-ink/68 transition hover:text-biome-accent">{item.label}</Link>)}
            </div>
          </nav>
          <nav aria-label="Fieldwork">
            ...
            <div className="mt-4 grid gap-3">
              {fieldwork.map((item) => (
                <Link key={item.href} href={item.href} target={item.external ? "_blank" : undefined} rel={item.external ? "noreferrer" : undefined} className="inline-flex items-center gap-1 text-sm text-biome-ink/68 transition hover:text-biome-accent">
        ...
        <div className="flex flex-col gap-3 border-t border-white/15 pt-6 text-[11px] text-biome-ink/45 sm:flex-row sm:items-center sm:justify-between">
