...
export function PlanLinksCard({ hotspot }: { hotspot: Hotspot }) {
  ...
  return (
    <section className="field-card rounded-sm p-6">
      ...
      <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Live links to help with directions, background, and booking — schedules and permit rules change by season, so confirm with the official source before you travel.</p>
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        {links.map(({ icon: Icon, label, href }) => (
          <a key={label} href={href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-sm border border-forest-700/15 bg-white/70 px-3 py-2.5 text-sm font-semibold text-forest-900 transition hover:border-forest-700/30 hover:bg-white dark:border-white/10 dark:bg-white/5 dark:text-slate-100 dark:hover:border-white/25 dark:hover:bg-white/10">
            <Icon size={16} className="text-forest-700 dark:text-forest-300" />
