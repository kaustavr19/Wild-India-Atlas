...
function LeadDestination({ hotspot }: { hotspot: Hotspot }) {
  ...
  return <article className={`biome-surface ${biomeClassName[biome]} grid overflow-hidden rounded-field border border-white/12 shadow-lift lg:grid-cols-[1.15fr_0.85fr]`}><HotspotImage slug={hotspot.slug} type={hotspot.type} className="h-72 w-full lg:h-full lg:min-h-[420px]" /><div className="texture-topography flex flex-col p-6 sm:p-8"><p className="field-label text-biome-accent">Field lead · {biome} · {hotspot.region}</p><h3 className="mt-4 font-display text-4xl leading-none text-biome-ink sm:text-5xl">{hotspot.name}</h3><p className="mt-3 flex items-center gap-1.5 text-xs text-biome-ink/48"><MapPin size={13} />{hotspot.state}</p><p className="mt-6 text-base leading-7 text-biome-ink/66">{hotspot.summary}</p><div className="mt-7 grid grid-cols-2 gap-4 border-y border-white/10 py-5"><FieldFact label="Best window" value={hotspot.bestMonths.slice(0, 5).join(" · ")} /><FieldFact label="Field signal" value={hotspot.mainSpecies.slice(0, 2).join(" · ")} /></div><div className="mt-auto flex flex-wrap items-center gap-3 pt-7"><Link href={`/hotspots/${hotspot.slug}`} className="atlas-button">Open field guide <ArrowRight size={14} /></Link><JournalSaveButton type="hotspot" slug={hotspot.slug} tone="dark" /></div></div></article>;
}
